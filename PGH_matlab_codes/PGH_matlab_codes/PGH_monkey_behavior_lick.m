...
is_outer_tube_success = logical(DLC.CLASS.is_r_reward_outer_tube_lick(LICKS_ALL_DATA.validity == 1) + DLC.CLASS.is_l_reward_outer_tube_lick(LICKS_ALL_DATA.validity == 1));
is_outer_tube_fail = logical(DLC.CLASS.is_r_noreward_outer_tube_lick(LICKS_ALL_DATA.validity == 1) + DLC.CLASS.is_l_noreward_outer_tube_lick(LICKS_ALL_DATA.validity == 1));
...
[peak,ind_peak,~,prom]= findpeaks(d_tip, 'MinPeakProminence', 1, 'MinPeakDistance',detection_window,'Annotate', 'extent');
...
for counter_peak = 1 : length(peak)
    ...
    if ~isempty(ind_lick_onset_)
        ind_lick_onset(counter_peak,1) =  ind_lick_onset_;
    else
        ind_lick_onset(counter_peak,1) = nan;
        ind_lick_offset(counter_peak,1) = nan;
    end
    if ~isempty(ind_lick_offset_)
...
is_nan = isnan(ind_lick_onset) | isnan(ind_lick_offset);
    ...
    function [x, y, ind_DLC_ERR] = find_erroneous_markers(x, y, type, ref)
        ...
        if eva.OptimalK ~= 1
            ...
            if sum(abs(diff(sort(c))) < 5) ~= (eva.OptimalK - 1)
                ...
                elseif type == "r"
                    if sum(dist>0) == 0
                        y_ref = ones(size(y))*min(abs(dist(find(dist * -1 > 0)))) + ref;
                    else
