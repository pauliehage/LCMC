function PGH_event_alignment_LED(path_to_raw, path_to_save, rec_name)
...
time_vid = ((1/VID.FPS) : (1/VID.FPS) : length(VID.LED_FPS(:,4))/VID.FPS);
...
if sample_diff > 0
    VID_LED_xcorr_LED_combined_1K  = VID_LED_combined_1K(abs(sample_diff):end);
    VID_LED_xcorr_time_1K                 = VID_time_1K(abs(sample_diff):end);
    EPHYS_LED_xcorr_LED_combined_1K = EPHYS_LED_combined_1K;
    EPHYS_LED_xcorr_time_1K                = EPHYS_time_1K;
elseif sample_diff < 0
    VID_LED_xcorr_LED_combined_1K  = VID_LED_combined_1K;
    VID_LED_xcorr_time_1K                 = VID_time_1K;
    EPHYS_LED_xcorr_LED_combined_1K = EPHYS_LED_combined_1K(abs(sample_diff):end);
    EPHYS_LED_xcorr_time_1K                = EPHYS_time_1K(abs(sample_diff):end);
end

% LED_combined: make the vectors the same size
if length(EPHYS_LED_xcorr_LED_combined_1K) ~= length(VID_LED_xcorr_LED_combined_1K)
    min_length = min([ length(EPHYS_LED_xcorr_LED_combined_1K),  length(VID_LED_xcorr_LED_combined_1K)]);
    VID_LED_xcorr_LED_combined_1K  = VID_LED_xcorr_LED_combined_1K(  1:min_length);
    VID_LED_xcorr_time_1K                 = VID_LED_xcorr_time_1K(                 1:min_length);
    EPHYS_LED_xcorr_LED_combined_1K = EPHYS_LED_xcorr_LED_combined_1K( 1:min_length);
...
VID_LED_xcorr_LED_combined_1K_filt  = filtfilt(b_butter,a_butter,VID_LED_xcorr_LED_combined_1K);
EPHYS_LED_xcorr_LED_combined_1K_filt = filtfilt(b_butter,a_butter,EPHYS_LED_xcorr_LED_combined_1K);
...
for counter_chunk = 1 : 1 : (length(ind_edges)-1)
    ...
    VID_LED_LED_combined_chunk  = VID_LED_xcorr_LED_combined_1K_filt(inds_chunk);
    EPHYS_LED_LED_combined_chunk = EPHYS_LED_xcorr_LED_combined_1K_filt(inds_chunk);
...
EPHYS.CH_EVE.align_LED.VID_LED_xcorr_LED_combined_1K       = VID_LED_xcorr_LED_combined_1K;
EPHYS.CH_EVE.align_LED.EPHYS_LED_xcorr_LED_combined_1K    = EPHYS_LED_xcorr_LED_combined_1K;
...
if EPHYS.debug_figures == 1
    ...
    hold on
    %ind
    plot(VID_LED_xcorr_LED_combined_1K ,'*-');
    plot(EPHYS_LED_xcorr_LED_combined_1K ,'o-')
    xlabel('Ind')
    ylim([0 1.1])
    title(['Aligned EPHYS/VID 100 | xcorr diff: ' num2str(sample_diff)  ' / ' num2str(sample_diff/1000) 's' ])
    legend('VID', 'EPHYS')
    subplot(3,1,2)
    hold on
    %time
    plot(EPHYS_LED_xcorr_time_1K , VID_LED_xcorr_LED_combined_1K ,'*-');
    plot(EPHYS_LED_xcorr_time_1K  ,EPHYS_LED_xcorr_LED_combined_1K ,'o-')
    legend('VID', 'EPHYS')
    xlabel('Time (s)')
    ylim([0 1.1])
    subplot(3,1,3)
    hold on
    %dtw
    plot(VID_LED_xcorr_LED_combined_1K(LED_ind_convert_from_VID_to_EPHYS) ,'*-');
    plot(EPHYS_LED_xcorr_LED_combined_1K ,'o-')
    xlabel('Ind')
    ylim([0 1.1])
    legend('VID', 'EPHYS')
